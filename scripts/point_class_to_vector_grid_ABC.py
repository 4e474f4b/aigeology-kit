...
# -*- coding: utf-8 -*-
"""
point_class_to_vector_grid_ABC.py

粗密のある「点の地形種別（クラス）」から、
ユーザー指定解像度のベクタポリゴングリッドを作成するスクリプト。
...
- ランダムフォレスト等で予測された点の地形クラスを、
  1m / 5m / 10m など任意解像度のベクタポリゴングリッドに変換したい。
- 元の予測点は HoldOut / k分割CV の都合で粗密があってもよい。
- 出力は GeoPackage 形式のベクタポリゴン（ラスタではない）であり、
  「セルごとに1クラスを持つ正方形ポリゴン」を格納する。
