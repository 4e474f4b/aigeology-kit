...
単一DEMから「比高・標準偏差・傾斜・曲率・開度」など
地形分類用の基本8特徴量を、マルチスケールで一括生成するスクリプト。
...
def main():
    ...
    if r_mode == 1:
        # --- r を「距離[m]」で指定するモード ---
        print("\n[比高・標準偏差のスケール指定]")
        print("  スケール指定の方法 r として ＜1: 距離 [m] で指定＞ を選択しました。\n")
        print("比高・偏差の計算範囲 R[m] を指定してください。")
        print("  例) 1mDEMで R=2 → 3×3窓, R=5 → 11×11窓")
        print("  ※ 解像度と k×k 窓の対応関係の詳細は、README の")
        print("     『解像度と窓サイズ (k×k) の関係』セクションを参照してください。\n")
        ...
        # --- r を「ピクセル数」で指定するモード ---
        print("\n[比高・標準偏差のスケール指定]")
        print("  スケール指定の方法 r として ＜2: ピクセル数 [px] で指定＞ を選択しました。\n")
        print("比高・偏差の計算範囲 r_px（ピクセル数）を指定してください。")
        print("  例) r_px=3 → 3×3窓, r_px=5 → 5×5窓 など")
        print("  ※ 解像度と k×k 窓の対応関係の詳細は、README の")
        print("     『解像度と窓サイズ (k×k) の関係』セクションを参照してください。\n")
