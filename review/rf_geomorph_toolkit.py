...
def _plot_learning_curve(
    ...
    if not hasattr(estimator, "named_steps") and hasattr(estimator, "estimator"):
        undersample_kwargs = {
            "target_n": estimator.target_n,
            "random_state": estimator.random_state,
        }
        estimator = estimator.estimator  # 内部 Pipeline を取り出す
    ...
    for n_estimators in est_list:
        ...
        for train_idx, valid_idx in cv.split(X, y):
            ...
            # アンダーサンプリングが有効だった場合は再ラップして fit 時に適用
            if undersample_kwargs is not None:
                model = UnderSampledClassifier(model, **undersample_kwargs)
            ...
            f1_scores_list.append(f1_score(y_val, y_pred, average="macro", zero_division=0))
    ...
    for r_path in rasters:
        ...
        try:
            with rasterio.open(r_path) as src:
                ...
                # float64 に変換しておく（整数型ラスターに NaN 代入するとValueError）
                vals = data[:, rows_safe, cols_safe].transpose(1, 0).astype(np.float64)
            ...
            print(
                f"  [WARN] ラスターの処理に失敗しました（スキップ）: {r_path}\n"
                f"         → {e}"
            )
            continue
...
def train_mode(backend: str = "rf"):
        ...
        if last_split is not None:
            ...
            if xy_cols is not None and eval_gpkg_thinning is not None and (save_gpkg or save_parquet):
                ...
                if (x_col in df.columns) and (y_col in df.columns):
                    ...
                    eval_gpkg_path = None  # save_gpkg=False の場合も参照できるよう初期化
                    if save_gpkg:
                    ...
                    # --- 学習データ側（train）の GPKG / Parquet も出力（オプション） ---
                    save_train_data = ask_yes_no(
                        "\n[オプション] 学習データ側（train_mc_last）も GPKG/Parquet で出力しますか？\n"
                        "  ※ 学習に使った行の座標・特徴量を確認したい場合に使用します。",
                        default=False,
                    )
    ...
    # ===== ホールドアウト法 =====
        ...
        if xy_cols is not None and eval_gpkg_thinning is not None:
            ...
            if (x_col in df.columns) and (y_col in df.columns):
                ...
                save_train_data = ask_yes_no(
                    "\n[オプション] 学習データ側（train_holdout）も GPKG/Parquet で出力しますか？\n"
                    "  ※ 学習に使った行の座標・特徴量を確認したい場合に使用します。",
                    default=False,
                )
    ...
    with open(latest_meta, "w", encoding="utf-8-sig") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2, default=_json_default)
    ...
    if est is not None and hasattr(est, "feature_importances_"):
        ...
        imp_df.to_csv(latest_imp, index=False, encoding="utf-8-sig")
...
def predict_mode():
    ...
    # 予測結果の保存先を「最初に」決めておく
    # =====================================================
    # 予測結果専用のサブディレクトリを作成（学習モードと同様の構成）
    in_stem = Path(in_path).stem
    default_pred_dir = Path(in_path).parent / f"{in_stem}_pred_{run_id}"

    print("\n[出力先の設定]")
    print(f"  デフォルト出力フォルダ: {default_pred_dir}")
    out_dir_in = input(
        "出力フォルダ（空=上記デフォルト。ディレクトリパスを指定可）: "
    ).strip().strip('"').strip("'")
    if not out_dir_in:
        out_dir = default_pred_dir
    else:
    ...
    out_fname = f"{in_stem}_pred_{run_id}"
    if low_in.endswith(".csv"):
        default_out_path = str(out_dir / f"{out_fname}.csv")
    elif low_in.endswith(".parquet") or low_in.endswith(".pq"):
        default_out_path = str(out_dir / f"{out_fname}.parquet")
    elif low_in.endswith(".gpkg"):
        default_out_path = str(out_dir / f"{out_fname}.gpkg")
    else:
        default_out_path = str(out_dir / f"{out_fname}.csv")
    ...
    if proba_max is not None:
        ...
        if ask_yes_no(
            ...
            # 閾値未満は欠損値としてマスク
            out["pred_label_masked"] = out["pred_label"].where(out["is_confident"])
    ...
    try:
        ...
        # 設定フェーズ（1ファイル出力に統合）
        # -------------------------------
        vec_gpkg_enable = False
        vec_parquet_enable = False
        vec_thinning = 1
        vec_x_col = None
        vec_y_col = None
        vec_crs_epsg = None
        add_is_correct = False

        if pred_df is not None and not pred_df.empty:
            if ask_yes_no("\n[オプション] 予測結果を保存しますか？", default=False):
                # 正解ラベルがある場合は is_correct 付加を尋ねる
                if eval_df is not None and not eval_df.empty:
                    add_is_correct = ask_yes_no(
                        "  → 正解ラベルが含まれています。正誤フラグ（is_correct）を付加しますか？",
                        default=False,
                    )
                vec_gpkg_enable, vec_parquet_enable = ask_vector_output_format(default="both")
                vec_thinning = ask_thinning_factor(
                    "  GPKG に書き出す間引き率を指定してください（例: 10=1/10, 100=1/100）",
                    default=1,
                )
                vec_x_col, vec_y_col = ensure_xy_columns(pred_df)
                vec_default_crs = source_crs_str or "EPSG:4326"
                vec_crs_epsg = ask_crs(default_epsg=vec_default_crs)
            else:
                print("  → ベクタ出力はスキップします。")
        else:
            print("\n[INFO] 予測結果が空のため、ベクタ出力はスキップします。")

        # -------------------------------
        # 書き出しフェーズ
        # -------------------------------
        if vec_gpkg_enable or vec_parquet_enable:
            # is_correct を付加する場合は eval_df を使用（true_label / is_correct 含む）
            vec_df = eval_df if add_is_correct and eval_df is not None else pred_df

            if vec_thinning and vec_thinning > 1:
                vec_out = vec_df.iloc[::vec_thinning].copy()
                print(
                    f"  → {vec_thinning} 行ごとに 1 行を採用して "
                    f"{len(vec_out)} 件を書き出します。"
                )
            else:
                vec_out = vec_df

            if vec_gpkg_enable:
                vec_gpkg_path = out_dir / f"{in_stem}_pred_{run_id}.gpkg"
                save_gpkg_with_points(
                    vec_out,
                    vec_gpkg_path,
                    x_col=vec_x_col,
                    y_col=vec_y_col,
                    crs_epsg=vec_crs_epsg,
                    layer_name="pred",
                )
                print(f"  → 予測結果 GPKG を保存しました: {vec_gpkg_path}")

            if vec_parquet_enable:
                vec_parquet_path = out_dir / f"{in_stem}_pred_{run_id}.parquet"
                try:
                    save_geoparquet_with_points(
                        vec_out,
                        vec_parquet_path,
                    ...
                    print(f"  → 予測結果 GeoParquet を保存しました: {vec_parquet_path}")
                except Exception as e:
    ...
    if low.endswith(".csv"):
        ...
        if vec_parquet_enable:
            # GeoParquet として既に保存済みのため plain parquet での上書きをスキップ
            print(f"\n✅ 予測結果（GeoParquet）を書き出しました: {out_path}")
        else:
            try:
                ...
                out.to_csv(out_fallback, index=False, encoding="utf-8-sig")
