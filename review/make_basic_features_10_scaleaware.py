...
【スケール指定の注意】
地形特徴量はすべて「中心セルと周辺セルの比較」で計算するため、
最小窓サイズは 3×3 ピクセルとなる。
入力 R[m] はピクセルサイズ px[m] で割って窓サイズ k に変換されるが、
k < 3 の場合は自動的に k=3 に切り上げられる。

  例）px=5m の場合
    R=5m  → k=round(5/5)=1  → k=3 に切り上げ（実効スケール 15m）
    R=15m → k=round(15/5)=3 → k=3（実効スケール 15m）
    ※ R=5m と R=15m は同一の計算になるため、重複指定に意味はない。

  最小スケールを求める場合は R = 3 × px を指定すること。
  例）px=5m → R_min=15m、px=1m → R_min=3m、px=0.5m → R_min=1.5m
...
def local_stddev(arr, win_pix: int):
    """局所標準偏差（NaN 対応：有効ピクセル数で正規化）"""
    k = int(max(1, win_pix))
    k2 = float(k * k)
    valid = np.isfinite(arr).astype(np.float32)
    a = np.where(np.isfinite(arr), arr, 0.0).astype(np.float32)
    # uniform_filter は k² で割った平均を返すため、k² を掛けて合計値に戻す
    count  = uniform_filter(valid,   size=k, mode="nearest") * k2  # 窓内の有効ピクセル数
    sum_z  = uniform_filter(a,       size=k, mode="nearest") * k2  # 窓内の合計値
    sum_z2 = uniform_filter(a * a,   size=k, mode="nearest") * k2  # 窓内の二乗合計値
    with np.errstate(invalid="ignore", divide="ignore"):
        mean   = np.where(count > 0, sum_z  / count, np.nan)
        mean_z2 = np.where(count > 0, sum_z2 / count, np.nan)
    var = mean_z2 - mean * mean
    var = np.where(np.isfinite(var), np.maximum(var, 0.0), np.nan)
    std = np.sqrt(var).astype(np.float32)
    std[~np.isfinite(arr)] = np.nan
    return std


def local_tpi(arr, win_pix: int):
    """TPI = z - mean(neighborhood)（NaN対応）"""
    m = smooth_box_nan(arr, win_pix)
...
def _clean_env_for_saga() -> dict:
    """
    conda 環境変数（GDAL_DRIVER_PATH 等）が SAGA の gflags と衝突するのを防ぐため、
    SAGA の実行に必要な最小限の環境変数のみを渡す。
    """
    return {
...
def compute_openness_with_saga(
    ...
    for r_open in openness_r_list:
        ...
        try:
            subprocess.run(cmd, check=True, env=_clean_env_for_saga())
...
def main():
    ...
    with rasterio.open(dem_path) as src:
        ...
        px = transform.a  # ピクセルサイズ（x方向）、正方形ピクセル前提
    ...
    print(f"  ※ このDEM（px={px:.1f}m）の最小有効スケールは R={3*px:.1f}m（3×3窓）です。それ以下の解像度を指定しても同じ結果になります。\n")
    ...
    if openness_mode != 3:
        ...
        # 開度半径に対応する外縁マスク幅を候補に追加する。
        # odd=True は _apply_valid_mask(r=k//2) との整合のため（開度計算には無関係）。
        for r_open in openness_r_list:
    ...
    print("  1: 生DEMのみで計算")
    print("  2: 生DEM + スムージング版も出力（推奨：比高スケールと揃える）")
    print("  3: スムージング版のみ出力")
    slope_mode = ask_int("番号を選んでください", 2)
